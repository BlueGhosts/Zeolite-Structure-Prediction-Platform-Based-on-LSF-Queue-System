...
function Si2SiO2Task(){
    ...
    for csqfile in $csqfiles
    do
        nowJobNumber=`bjobs -J *Si2SiO2* | wc -l`
        while [[ $nowJobNumber -gt 2 ]]
        do
            sleep 30s
            nowJobNumber=`bjobs -J *Si2SiO2* | wc -l`
        done
...
if [ ! -d $wycpath/AllGULP_gin ]; then
    mkdir $wycpath/AllGULP_gin
fi
rm -rf $wycpath/AllGULP_gin/*

if [ ! -d $wycpath/AllGULP_gout ]; then
    mkdir $wycpath/AllGULP_gout
fi
rm -rf $wycpath/AllGULP_gout/*
