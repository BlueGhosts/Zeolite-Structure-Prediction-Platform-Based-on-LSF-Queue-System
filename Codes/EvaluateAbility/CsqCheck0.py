...
class CoordinateSequenceIF():
    def __init__(self, name, atomcsqs, referenceCsqs, filename, csqRequirements = {}):
        self.name = name
        ...
        self.isgood = self.Judge(csqRequirements)
    ...
    def Judge(self, csqRequirements = {}):
        if csqRequirements:
            for csqRequirement in csqRequirements:
                if csqRequirement <= self.csqnum - 1:
                    for atomcsq in self.atomcsqs:
                        if int(atomcsq[csqRequirement]) < csqRequirements[csqRequirement][0] or int(atomcsq[csqRequirement]) > csqRequirements[csqRequirement][1]:
                            return False
        return True
...
def ReadCsqFile(filename, csqRequirement = {}, selfcheck = False, referenceCsqs = []):
    def ReadOneCsq(file, filename, csqRequirement = {}, referenceCsqs = []):
        ...
        structure = CoordinateSequenceIF(name, atomcsqs, referenceCsqs, filename, csqRequirement)
        return structure
...
def ReadInputFile(filename):
    ...
    # print()
    return csq_requirement, ycsqFilenames, ncsqFilenames, referenceCsqFilenames, outtxtFilename, outAllCsqFilename


def CsqCheck(filename):
    csq_requirement, ycsqFilenames, ncsqFilenames, referenceCsqFilenames, outtxtFilename, outAllCsqFilename = ReadInputFile(filename)
    Allstructures = []
    ...
    for csqFile in ycsqFilenames:
        structures = ReadCsqFile(csqFile, csq_requirement, True, referenceCsqs)
        Allstructures = Allstructures + structures
    for csqFile in ncsqFilenames:
        structures = ReadCsqFile(csqFile, csq_requirement, False, referenceCsqs)
        Allstructures = Allstructures + structures
    # for csq in Allstructures:
    #     print(csq.name)
    #     print(csq.id)
