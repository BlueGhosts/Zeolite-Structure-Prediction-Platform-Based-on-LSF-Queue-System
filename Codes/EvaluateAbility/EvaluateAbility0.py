...
def EvaluateAbility(csqParaFilename, outFilename):
    csq_requirement, ycsqFilenames, ncsqFilenames, referenceCsqFilenames, outtxtFilename, outAllCsqFilename = CsqCheck.ReadInputFile(csqParaFilename)
...
def RebuildFraGen(overwycs, path, newpath, eachWycCycleNumber, referencefiles, neweachWycCycleNumber):
    ...
    inpfilenames = Set(wycfilename, newpath, inpfilename.split('/')[-1].split('\\')[-1].split('.')[0] + '+', inpfilename, len(overwycs) + 1, eachWycCycleNumber, wycfilename)
...
if __name__ == '__main__':
    ...
    csq_requirement, ycsqFilenames, ncsqFilenames, referenceCsqFilenames, outtxtFilename, outAllCsqFilename = CsqCheck.ReadInputFile(
        csqParaFilename)
    ...
    if overwycs:
        # print(True)
        inpfilenames = RebuildFraGen(overwycs, path, path + '+', eachWycCycleNumber, oldcsqwfilenames, eachWycCycleNumber + oldeachWycCycleNumber)
        for inpfilename in inpfilenames:
