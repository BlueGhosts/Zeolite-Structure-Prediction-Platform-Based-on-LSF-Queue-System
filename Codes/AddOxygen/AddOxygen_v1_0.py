...
# AddOxygen For One file v1.0
# by wangjiaze on 2019-Apr-15
# For One file, For 166
...
def CalculateOxygens(mindistance, maxdistance, GroupName, CellParameter, Atom_Coordinate):
    ...
    def Add0xygenAtom(mindistance, maxdistance, Limit27CellCoordinate):
        ...
        for i in range(len(Limit27CellCoordinate)):
            for j in range(i + 1, len(Limit27CellCoordinate)):
                ...
                distance = CoorTrans.CalculateDistance(CoorTrans.Fractional2Cartesian(coordinate1, CellParameter), CoorTrans.Fractional2Cartesian(coordinate2, CellParameter, 6))
    ...
    OxygenCoordinate = Add0xygenAtom(mindistance, maxdistance, Limit27CellCoordinate)
...
def AddFileOxygen(filename, outpath, mindistance, maxdistance):
    def ReadSFile(filename):
        ...
        Lit = file.readline().strip()
        GroupName = file.readline().strip()
        Parameter = file.readline().split()
        CellParameter = []
        for a in Parameter:
            CellParameter.append(float(a))
        Atom_Coordinate = {}
        while True:
            line = file.readline()
            if '-' in line:
            ...
            line = line.split()
            Atom = line[0]
            Coordinate = [float(line[1]), float(line[2]), float(line[3])]
            Atom_Coordinate[Atom] = Coordinate
        return Name, Lit, GroupName, CellParameter, Atom_Coordinate


    def Write2csfile(filename, strname, lit, groupname, cellparameter, Atom_SiCoordinates, Atom_OxygenCoordinates):
        file = open(filename, 'w')
        file.write('\n')
        file.write(' Name: ' + strname + '\n')
        file.write(' Lit: ' + lit + '\n')
        file.write('\n')
        file.write(' Cell parameters:  a = '+ str(cellparameter[0]) +'  b = '+ str(cellparameter[1]) +'  c = '+ str(cellparameter[2]) +'\n')
        file.write('                   alpha = '+ str(cellparameter[3]) +'  beta = '+ str(cellparameter[4]) +'  gamma = '+ str(cellparameter[5]) +'\n')
        file.write('                   volume = '+ str(CoorTrans.CalculateVolume(cellparameter)) +'\n')
        file.write('\n')
        file.write(' Space group: '+ groupname +'  ('+ str(CoorTrans.Groupname2Groupnum(groupname)) +')\n')
        file.write('\n')
        file.write(' Atoms with coordination number and atoms per unit cell\n')

        form = " ({0:>3})  {1:<3}  ({2:<.6f}  {3:<.6f}  {4:<.6f}){5:>4}{6:>4}{7:>10}\n"
        i = 1
        for Atom, Coordinates in Atom_SiCoordinates.items():
            Coordinate = Coordinates[0]
            file.write(form.format(str(i), Atom, Coordinate[0], Coordinate[1], Coordinate[2], 4, len(Coordinates), 'pass'))
            i = i + 1
        for Atom, Coordinates in Atom_OxygenCoordinates.items():
            Coordinate = Coordinates[0]
            file.write(form.format(str(i), Atom, Coordinate[0], Coordinate[1], Coordinate[2], 2, len(Coordinates), 'pass'))
            i = i + 1
        file.close()
    ...
    def Write2ciffile(filename, groupname, cellparameter, Atom_SiCoordinates, Atom_OxygenCoordinates):
        ...
        if CoorTrans.Groupname2Groupnum(groupname) == 166:
            file.write('loop_\n')
            file.write('_symmetry_equiv_pos_as_xyz\n')
            file.write('x,y,z\n')
            file.write('x+ 0.5,y+ 0.5,z+ 0.5\n')
            file.write('-x+ 0.5,-y,z+ 0.5\n')
            file.write('-x,-y+ 0.5,z\n')
            file.write('-x,y+ 0.5,-z+ 0.5\n')
            file.write('-x+ 0.5,y,-z\n')
            file.write('x+ 0.5,-y+ 0.5,-z\n')
            file.write('x,-y,-z+ 0.5\n')
            file.write('-x,-y,-z\n')
            file.write('-x+ 0.5,-y+ 0.5,-z+ 0.5\n')
            file.write('x+ 0.5,y,-z+ 0.5\n')
            file.write('x,y+ 0.5,-z\n')
    ...
    StrName, Lit, GroupName, CellParameter, Atom_Coordinate = ReadSFile(filename)
    ...
    print('AddOxygen ' + sfilename + '>' + outpath + '/' + sfilename.split('/')[-1].split('.')[0] + '.cif' + ' done!')
...
def AddOxygen(inpath, outpath, logfilename, mindistance, maxdistance):
    ...
    sfilenames = ExtractInformation.GetFilenames(inpath, 's')
    for sfilename in sfilenames:
        AddFileOxygen(sfilename, outpath, mindistance, maxdistance)


        print('AddOxygen ' + sfilename + '>' + outpath + '/' +sfilename.split('/')[-1].split('.')[0] + '.s.cs' + ' done!')
        logfile.write('AddOxygen ' + sfilename + '>' + outpath + '/' +sfilename.split('/')[-1].split('.')[0] + '.s.cs' + ' done!\n')
