...
# python Separate V1.2
# by wangjiaze on 2019-Sep-8
...
def set(wycfilename, outpath, label, referenceinp, n, cyclenum):
    ...
    while True:
        ...
        for i in range(n):
            ...
            if not line.strip():
                outfile.close()
                inpfilename = dealfile(outname, referenceinp, n, cyclenum)
                if inpfilename not in inpfilenames:
...
if __name__ == '__main__':
    ...
    inpfilenames = set(wycfilename, outpath, label, referenceinp, maxnum, cyclenum)
